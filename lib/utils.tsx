  ...
  try {
    ...
    if (saved) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        // Auto-update check: if the user has the old dummy data (less than 19 items), replace it
        if (parsed.length < 19) {
          console.log("Old dummy data detected, updating to new richer dummy data...");
          const { dummySubmissions } = require("./dummy-data");
    ...
    // Fallback to dummy data if local storage is empty or empty array
    if (typeof window !== 'undefined') {
      console.log("No local data found, initializing with dummy data...");
      const { dummySubmissions } = require("./dummy-data");
      localStorage.setItem("submissions", JSON.stringify(dummySubmissions));
      return dummySubmissions;
    }
